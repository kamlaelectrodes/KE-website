document.addEventListener('DOMContentLoaded', () => {
  ...
  loadCss('site-fixes.css?v=5');
  loadCss('photo-layout-fix.css?v=2');
  loadCss('hygiene-v4.css?v=1');
  loadCss('experience-v5.css?v=1');

  /* Convert embedded facility-photo variables into real img elements for Safari reliability. */
  ...
  const hydrateFacilityPhotos = () => {
    ...
    ].forEach(({ className, variable, width, height }) => {
  ...
  /* Create the second navigation row only on mobile/tablet. Desktop never receives it. */
  ...
  if (pageName === 'faq.html') document.querySelector('aside.callout, aside.card')?.classList.add('support-rail');
  ...
  const answers = {
    ...
    distributorship:'<strong>Distributorship enquiry</strong><br>Share firm name, market area, current lines and monthly capacity. <br><a href="become-a-dealer.html">Apply for Distributorship →</a>',
    products:'<strong>Product help</strong><br>Open Product Range and expand any product block for usage and technical information. <br><a href="products.html">View products →</a>',
  ...
  /* Scroll-linked fixed gear and back-to-top visibility. */
  ...
  /* Motion rail on the homepage, inspired by modern industrial sites without copying their design. */
  if (pageName === 'index.html' && !document.querySelector('.motion-rail')) {
    ...
    if (hero) {
      ...
      const sequence = [...items,...items].map(item => `<span>${item}</span>`).join('');
      rail.innerHTML = `<div class="motion-rail-track">${sequence}</div>`;
      hero.insertAdjacentElement('afterend', rail);
    }
  }

  /* Progressive reveal with immediate fallback for browsers without IntersectionObserver. */
  const revealTargets = document.querySelectorAll('.section .card, .section .callout, .industry-story, .csr-story, .facility-visual, .stock-visual');
