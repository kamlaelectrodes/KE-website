#!/usr/bin/env python3
"""Static, live-HTTP and real Web3Forms audit for kamlaelectrodes.com."""
...
UTILITY_WITHOUT_SHARED_APP = {"404.html", "thank-you.html"}
...
def local_target(source: Path, raw: str) -> tuple[Path | None, str]:
    ...
    if (
        not value
        or value.startswith(SKIP_SCHEMES)
        or value.startswith("//")
        or parsed.scheme
        or parsed.netloc
    ):
    ...
    target = ROOT / clean.lstrip("/") if clean.startswith("/") else (source.parent / clean).resolve()
    if target.is_dir():
...
def static_audit() -> tuple[list[Finding], list[dict]]:
    ...
    for page in pages:
        ...
        app_script = soup.find("script", src=lambda value: value and "app.js" in value)
...
def live_audit(base_url: str, cache_key: str) -> list[Finding]:
    findings: list[Finding] = []
    ...
    for page in html_files():
        if page.name == "404.html":
            continue
        relative = "" if page.name == "index.html" else page.name
        url = urljoin(base_url.rstrip("/") + "/", relative)
        url += ("&" if "?" in url else "?") + f"qa={cache_key}"
        try:
            response = session.get(url, timeout=30, allow_redirects=True)
        except requests.RequestException as exc:
            findings.append(Finding("error", page.name, f"Live request failed: {exc}"))
            continue
        if response.status_code != 200:
            findings.append(Finding("error", page.name, f"Live HTTP status {response.status_code} at {response.url}"))
            continue
        soup = BeautifulSoup(response.text, "html.parser")
        if not soup.title or not soup.title.get_text(strip=True):
            findings.append(Finding("error", page.name, "Live page has no title"))
...
def field_value(name: str, control, timestamp: str) -> str:
    key = name.lower()
    fixed = {
    ...
    if control_type == "email" or "email" in key:
        ...
        for form in soup.find_all("form"):
            ...
            for control in form.find_all(["input", "select", "textarea"]):
                ...
                if not value or name in {"subject", "from_name"}:
                    value = field_value(name, control, timestamp)
                data[name] = value
...
def main() -> int:
    ...
    findings.extend(live_audit(args.live_base, timestamp))
    subjects: list[str] = []
    if args.send_forms:
    ...
    report = {
        ...
        "pages_checked": [page.name for page in html_files()],
        "forms": forms,
        "test_submission_subjects": subjects,
        "findings": [asdict(item) for item in findings],
    }
    (ROOT / args.report).write_text(json.dumps(report, indent=2, ensure_ascii=False), encoding="utf-8")
